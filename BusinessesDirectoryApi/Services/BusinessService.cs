...
namespace BusinessesDirectoryApi.Services
{
    public class BusinessService : IBusinessService
    {
      ...
      public async Task<BusinessDto> AddABusiness(BusinessToCreateDto businessToCreateDto)
      {
        ...
        if (city == null)
        {
          throw new Exception(String.Format("La ciudad indicada no pudo ser encontrada.", businessToCreateDto.CityId));
        }
        var state = await _locationRepository.FindStateById(Guid.Parse(businessToCreateDto.StateId));
        if (state == null)
        {
          throw new Exception(String.Format("El estado no se encontró.", businessToCreateDto.StateId));
        }
        else if (state.StateId != city.StateId)
        {
          throw new Exception(String.Format("La ciudad no es parte del estado indicado.", state.StateId));
        }
        var country = await _locationRepository.FindCountryById(Guid.Parse(businessToCreateDto.CountryId));
        if (country == null)
        {
          throw new Exception(String.Format("El país no se encontró.", businessToCreateDto.CountryId));
        }
        if (country.CountryId != city.CountryId)
        {
          throw new Exception(String.Format("La ciudad no es parte del país indicado.",country.CountryId));
        }
        if (country.CountryId != state.CountryId)
        {
          throw new Exception(String.Format("El estado no es parte de ese país indicado..", country.CountryId));
        }
